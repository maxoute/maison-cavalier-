...
export function NotificationTemplates({ templates }: { templates: NotificationTemplate[] }) {
  ...
  return (
    <div className="space-y-4 text-[12.5px]">
      {templates.length === 0 && (
        <p className="text-[11px] text-grey">Aucun modèle propre à cet immeuble : les concierges utilisent les modèles intégrés (travaux, incident technique, événement).</p>
      )}
      {templates.map(template => (
        <article key={template.id} className="rounded-[8px] border border-navy-3 bg-navy-2/60 px-4 py-3 space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h3 className="text-[13px] text-cream">{template.label}</h3>
            <div className="flex items-center gap-3">
              <button type="button" className="text-[11px] text-cream/80 underline underline-offset-4 cursor-pointer"
          ...
          <p className="text-cream/90">{template.title}</p>
          <p className="whitespace-pre-wrap break-words text-[11.5px] text-grey">{template.body}</p>
        </article>
      ))}
      <div className="rounded-[8px] border border-navy-3 px-4 py-3">
        <h3 className="mb-3 text-[13px] text-cream">{edited ? `Modifier « ${edited.label} »` : 'Nouveau modèle'}</h3>
        <OperationForm key={edited?.id ?? 'nouveau'} action={saveNotificationTemplate} submit={edited ? 'Mettre à jour le modèle' : 'Ajouter le modèle'}>
          ...
          <p className="text-[11px] text-grey">Les éléments entre crochets, par exemple [date], sont complétés par le concierge avant diffusion. Réutiliser un nom existant met le modèle à jour.</p>
        </OperationForm>
      </div>
    </div>
  );

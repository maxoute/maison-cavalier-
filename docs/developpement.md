...
| §6.1.2 Live Map | Contrat `DriverPositionProvider`, flux simulé déterministe (4 chauffeurs, latence < 1 s), carte SVG, sélection/suivi, filtres, fiche chauffeur, courses réelles — vérifié le 22/09 | Fournisseur Mapbox + positions GPS réelles des chauffeurs |
| §6.1.3 Voiturier | Table courses affichée sur la Live Map ; Plan B (alerte après 3 min sans réponse, taxi externe) simulé côté client | Attribution automatique/manuelle persistée, acceptation chauffeur, notifications |
...
| §6.2 Portail syndic | Messagerie, documents et reporting mensuel agrégé (fonction SQL `security definer`, isolation testée) — vérifié le 22/09 | Notifications urgentes réelles |
| §6.3.1 Dashboard global | KPIs consolidés (demandes, revenus, SLA, satisfaction, incidents) et comparaison multi-immeubles avec bascule — vérifié le 22/09 | Séries temporelles détaillées, alertes configurables |
| §6.3.2 Immeubles | Parc, indicateurs par immeuble, onboarding en une étape (immeuble + concierge + syndic) via service-role, immeuble piloté suivi par tous les écrans — vérifié le 22/09 | Exports par immeuble |
| §6.3.3 Utilisateurs | Liste des comptes staff, création, désactivation/réactivation via `auth.admin`, autorisations par rôle, journal d’audit — vérifié le 22/09 | Journal consultable dans l’interface, rôles granulaires |
...
| §6.3.5 Finance | KPIs mensuels, tendance six mois, répartition par service et par immeuble, export CSV — vérifié le 22/09 | Transactions Stripe, remboursements/litiges, reversements réels |
...
| §7.4 Reporting | Reporting mensuel syndic agrégé (SQL sécurisé) et finance admin — vérifiés le 22/09 | Génération/envoi mensuel automatique, exports PDF |
